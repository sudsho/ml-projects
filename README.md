...
|---------|-------------|--------|
| [Mixture Density Networks for Multimodal Regression](./mixture-density-networks/) | An MDN from scratch in PyTorch - a network emitting Gaussian mixture parameters instead of a point estimate, trained by NLL, on an inverse problem whose conditional is genuinely multimodal. Day 1 is the accounting for why a point estimate is the wrong object here: it returns a legitimate preimage on the most probable branch, looks correct under every point metric, and silently discards 56% of the conditional mass | 🔄 In Progress |
