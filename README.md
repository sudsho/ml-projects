...
|---------|-------------|--------|
| [Particle Filtering for Nonlinear State-Space Models](./particle-filter-nonlinear/) | Where the Kalman project stops. Sequential Monte Carlo from scratch in NumPy - the bootstrap filter on a state-space model whose observation is even in the state, so the posterior is genuinely bimodal and no Gaussian belief can represent it. Weight degeneracy and the effective sample size, multinomial vs systematic vs stratified resampling, the extended and unscented Kalman filters on the same tracks, and backward-simulation smoothing, all scored against an exact grid filter rather than against the truth | 🔄 In Progress |
